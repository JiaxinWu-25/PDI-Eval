...
def _run_sequence(args):
    ...
    try:
        # Derive click point from center of first frame
        ...
        report = pipeline.run(video_path=video_path, click_points=click_points, render_output_dir=str(seq_out))
...
def _has_pipeline_cache(video_id: str, cache_dir: str = "output/cache") -> bool:
    """Return True if all three pipeline steps are cached for this video."""
    base = Path(cache_dir)
    return all(
        (base / f"{video_id}_{step}.npz").exists()
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="results/gt_eval")
    ...
    for category, seq_id, video_path in entries:
        ...
        if result is not None:
            print(f"[CACHE] Loaded existing report: {seq_key}")
            cached_rows.append((seq_key, category, result, None))
        else:
            has_npz = _has_pipeline_cache(seq_id)
    ...
    print(f"[INFO] Cached reports: {len(cached_rows)}  |  Pending: {len(pending_entries)}  |  GPUs: {gpus}")
