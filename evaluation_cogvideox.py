"""
Camera video batch evaluation for PDI-Eval.

Scans the camera_video folder (structure: <model>/<category>/<index>/<filename>.mp4),
runs the PDI evaluation pipeline in parallel across multiple GPUs for both Flow and
Sora models sequentially, and writes a formatted summary table grouped by category.

Usage:
    python evaluation_cogvideox.py [--camera_video_dir camera_video]
                                   [--config configs/default.yaml]
                                   [--output_dir results/camera_eval]
                                   [--gpus 4,5,6,7]
                                   [--models Flow,Sora]
"""
...
# ---------------------------------------------------------------------------
# Discover all videos in the camera_video/<model> folder
# ---------------------------------------------------------------------------

def _discover_videos(model_dir: str):
    """
    Returns a list of (category, seq_id, video_path) tuples.
    Directory structure: <model_dir>/<category>/<index>/<filename>.mp4
    seq_id is the numeric index folder name (e.g. "1", "2", ...).
    """
    entries = []
    base = Path(model_dir)
    for cat_dir in sorted(base.iterdir()):
        ...
        for idx_dir in sorted(
            ...
            mp4_files = list(idx_dir.glob("*.mp4"))
            if mp4_files:
                entries.append((cat_dir.name, idx_dir.name, str(mp4_files[0])))
    return entries
...
def _run_sequence(args):
    ...
    try:
        ...
        config["cache_dir"] = str(_Path(output_dir) / "cache")
...
def _write_table(rows: list, output_path: str, title: str = "Camera Video") -> str:
    W = 146
    lines = []
    lines.append("=" * W)
    lines.append(f"  {title} - PDI Evaluation Summary")
...
def _run_model_eval(model: str, model_dir: str, config_path: str, output_dir: str,
                    src_dir: str, gpus: list, all_prompts: dict):
    """Run PDI evaluation for a single model and write its summary table."""
    print(f"\n{'=' * 60}")
    print(f"[INFO] Evaluating model: {model}")
    print(f"{'=' * 60}")

    entries = _discover_videos(model_dir)
    if not entries:
        print(f"[ERROR] No videos found under: {model_dir}")
        return

    print(f"[INFO] Found {len(entries)} videos across {len(set(e[0] for e in entries))} categories")

    # Build prompt map: seq_id is a 1-based numeric string -> index into camera_objects.json
    prompt_map = {}
    for category, seq_id, _ in entries:
        idx = int(seq_id) - 1 if seq_id.isdigit() else 0
        cat_prompts = all_prompts.get(category, [])
        prompt_map[(category, seq_id)] = cat_prompts[idx] if 0 <= idx < len(cat_prompts) else None
    ...
    table_path = os.path.join(output_dir, f"pdi_results_{model.lower()}_{ts}.txt")
    content = _write_table(rows, table_path, title=f"{model} Camera Video")
...
def main():
    parser = argparse.ArgumentParser(description="Camera Video Batch PDI Evaluation (Flow & Sora)")
    parser.add_argument("--camera_video_dir", type=str, default="camera_video",
                        help="Path to camera_video folder (structure: <model>/<category>/<index>/<file>.mp4)")
    parser.add_argument("--config",           type=str, default="configs/default.yaml")
    parser.add_argument("--output_dir",       type=str, default="results/camera_eval")
    parser.add_argument("--gpus",             type=str, default="4,5,6,7",
                        help="Comma-separated GPU IDs")
    parser.add_argument("--models",           type=str, default="Flow,Sora",
                        help="Comma-separated model names (subfolder names under camera_video_dir)")
    args = parser.parse_args()

    gpus = [int(g) for g in args.gpus.split(",")]
    models = [m.strip() for m in args.models.split(",")]

    camera_video_dir = Path(args.camera_video_dir).resolve()
    config_path      = str(Path(args.config).resolve())
    base_output_dir  = Path(args.output_dir).resolve()
    src_dir          = str(Path(__file__).parent / "src")

    # Load object names once (shared across models)
    ...
    for model in models:
        model_dir    = str(camera_video_dir / model)
        model_out    = str(base_output_dir / model.lower())
        _run_model_eval(model, model_dir, config_path, model_out, src_dir, gpus, all_prompts)

    print("\n[INFO] All models evaluated.")
